...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    if(this != NULL && nodeIndex >= 0 && nodeIndex <= ll_len(this))
    {
        ...
        if(newNode != NULL)
        {
            ...
            {
                auxiliary = getNode(this, nodeIndex - 1);

                if(auxiliary != NULL)
                {
                    newNode->pNextNode = auxiliary->pNextNode;
                    auxiliary->pNextNode = newNode;
                    this->size++;
                    returnAux = 0;
                }
            }
...
int ll_add(LinkedList* this, void* pElement)
{
    ...
    return addNode(this, accu, pElement);
}
                ...
                for(int j = i + 1; j < ll_len(this); j++)
                {
                    if(order)
                    {
                        if(pFunc(ll_get(this, i), ll_get(this, j)) == 1)
                        {
                            ...
                            getNode(this, i)->pElement = ll_get(this, j);
                            getNode(this, j)->pElement = pElement;
                        }
                    }

                    else if(pFunc(ll_get(this, i), ll_get(this, j)) == -1)
                        {
                            pElement = ll_get(this, i);
                            getNode(this, i)->pElement = ll_get(this, j);
                            getNode(this, j)->pElement = pElement;
